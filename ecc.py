...
class S256PField(FieldElement):
    prime = P

    def __init__(self, num, prime=None):
        super().__init__(num=num, prime=self.prime)
...
class S256PPoint(Point):
    bits = 256
    order = N

    def __init__(self, x, y, a=None, b=None):
        a, b = S256PField(A), S256PField(B)
        if x is None:
            super().__init__(x=None, y=None, a=a, b=b)
        elif type(x) == int:
            super().__init__(x=S256PField(x), y=S256PField(y), a=a, b=b)
        else:
            super().__init__(x=x, y=y, a=a, b=b)

    def __repr__(self):
        if self.x is None:
            return 'S256PPoint(infinity)'
        else:
            return 'S256PPoint({:x},{:x})_{:x}'.format(self.x.num, self.y.num, self.x.prime)

    def __rmul__(self, coefficient):
        # we want to mod by N to make this simple
        coef = coefficient % self.order
        # current will undergo binary expansion
        current = self
        # result is what we return, starts at 0
        result = S256PPoint(None, None)
    ...
    def verify(self, z, sig):
        ...
        s_inv = pow(sig.s, self.order - 2, self.order)
        # u = z / s
        u = z * s_inv % self.order
        # v = r / s
        v = sig.r * s_inv % self.order
    ...
    def parse(self, sec_bin):
        ...
        if sec_bin[0] == 4:
            ...
            return S256PPoint(x=x, y=y)
        is_even = sec_bin[0] == 2
        x = S256PField(int(sec_bin[1:].hex(), 16))
        # right side of the equation y^2 = x^3 + 7
        alpha = x**3 + S256PField(B)
        # solve for left side
        beta = alpha.sqrt()
        if beta.num % 2 == 0:
            even_beta = beta
            odd_beta = S256PField(P - beta.num)
        else:
            even_beta = S256PField(P - beta.num)
            odd_beta = beta
        if is_even:
            return S256PPoint(x, even_beta)
        else:
            return S256PPoint(x, odd_beta)
...
class S256PTest(TestCase):
    ...
    def test_pubpoint(self):
        ...
        for secret, x, y in points:
            # initialize the secp256k1 point (S256PPoint)
            point = S256PPoint(x, y)
...
class PrivateKey:
    ...
    def sign(self, z):
        ...
        k_inv = pow(k, self.order - 2, self.order)
        # s = (z+r*secret) / k
        s = (z + r*self.secret) * k_inv % self.order
        if s > self.order/2:
            s = self.order - s
...
class S256QField(FieldElement):
    ...
    def sqrt(self):
        return modular_sqrt(self.num, self.prime)


class S256QPoint(Point):
    bits = 256
    order = P

    def __init__(self, x, y, a=None, b=None):
        a, b = S256QField(A), S256QField(B)
        if x is None:
            super().__init__(x=None, y=None, a=a, b=b)
        elif type(x) == int:
            super().__init__(x=S256QField(x), y=S256QField(y), a=a, b=b)
        else:
            super().__init__(x=x, y=y, a=a, b=b)

    def __repr__(self):
        if self.x is None:
            return 'S256QPoint(infinity)'
        else:
            return 'S256QPoint({:x},{:x})_{:x}'.format(self.x.num, self.y.num, self.x.prime)

    def __rmul__(self, coefficient):
        # we want to mod by P to make this simple
        coef = coefficient % self.order
        # current will undergo binary expansion
        current = self
        # result is what we return, starts at 0
        result = S256QPoint(None, None)
        # we double 256 times and add where there is a 1 in the binary
        # representation of coefficient
        for i in range(self.bits):
            if coef & 1:
    ...
    def parse(self, sec_bin):
        ...
        if sec_bin[0] == 4:
            x = int(sec_bin[1:33].hex(), 16)
            y = int(sec_bin[33:65].hex(), 16)
            return S256QPoint(x=x, y=y)
        is_even = sec_bin[0] == 2
        x = S256QField(int(sec_bin[1:].hex(), 16))
        # right side of the equation y^2 = x^3 + 7
        alpha = x**3 + S256QField(B)
        # solve for left side
        beta = alpha.sqrt()
        if beta.num % 2 == 0:
            even_beta = beta
            odd_beta = S256QField(P - beta.num)
        else:
            even_beta = S256QField(P - beta.num)
            odd_beta = beta
        if is_even:
            return S256QPoint(x, even_beta)
        else:
            return S256QPoint(x, odd_beta)


# Per Andrew Poelstra
G2 = S256QPoint(
    0x76c39f5585cb160eb6b06c87a2ce32e23134e45a097781a6a24288e37702eda6,
    0x3ffc646c7b2918b5dc2d265a8e82a7f7d18983d26e8dc055a4120ddad952677f)
